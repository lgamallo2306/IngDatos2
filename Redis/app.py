...
def login():
    ...
    token = repo.crear_sesion(
        user_id=usuario_id, username=username, ip_address=ip_cliente
    )
...
def ver_feed():
    ...
    if sesion:
        ...
        return jsonify({"mensaje": "Acceso autorizado al Feed", "usuario": sesion}), 200
    else:
...
def inspeccionar_sesion():
    ...
    if not token:
        return (
            jsonify({"error": "Se requiere el 'token' para realizar la inspección"}),
            400,
        )
    ...
    if sesion:
        ...
        return (
            jsonify({"error": "La sesión no existe, es inválida o ya ha expirado"}),
            404,
        )
...
def desbanear_usuario():
    ...
    if exito:
        return (
            jsonify(
                {
                    "mensaje": f"IP {ip_a_desbanear } ha sido desbaneada con éxito y vuelve a tener acceso."
                }
            ),
            200,
        )
        ...
        return (
            jsonify(
                {
                    "mensaje": f"La IP {ip_a_desbanear } no se encontraba en la lista negra."
                }
            ),
            404,
        )
