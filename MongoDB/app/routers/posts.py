...
async def search_posts(q: str = Query(..., min_length=1)):
    db = get_database()
    cursor = (
        db.posts.find({"$text": {"$search": q}}, {"score": {"$meta": "textScore"}})
...
async def posts_by_tag(tag: str, limit: int = Query(20, le=100)):
    db = get_database()
    cursor = db.posts.find({"tags": tag}).sort("created_at", -1).limit(limit)
    ...
    pipeline = [
        ...
        {"$limit": limit},
        {
            "$lookup": {
                ...
                "as": "author",
                "pipeline": [
                    {"$project": {"username": 1, "display_name": 1, "avatar_url": 1}}
                ],
            }
        },
...
async def posts_by_user(
    user_id: str, page: int = Query(1, ge=1), limit: int = Query(10, le=50)
):
    db = get_database()
    skip = (page - 1) * limit
...
async def like_post(post_id: str):
    db = get_database()
    result = await db.posts.update_one({"_id": post_id}, {"$inc": {"likes_count": 1}})
